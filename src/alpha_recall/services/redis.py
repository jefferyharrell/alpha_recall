...
class RedisService:
    ...
    def update_self_prompt(self, message: str) -> dict[str, Any]:
        """Update the intentional memory (self-prompt) for maintaining project continuity.

        This stores a single natural language message to myself that gets displayed
        at the top of gentle_refresh to answer "What was I just about to do?"

        Args:
            message: Natural language instructions to myself. Empty string clears it.
        ...
        try:
            intent_key = "alpha:current_focus"

            if message.strip() == "":
                # Empty string means clear the intentional memory
                result = self.client.delete(intent_key)
                operation = "cleared"
                logger.info(
                    "Intentional memory cleared",
                    key_existed=bool(result),
                    correlation_id=correlation_id,
                )
            else:
                # Store the intentional memory message
                self.client.set(intent_key, message)
                operation = "set"
                logger.info(
                    "Intentional memory updated",
            ...
            return {
                "success": True,
                "operation": operation,
                "message": message if operation == "set" else None,
                "created_at": time_service.now()["iso_datetime"],
                "operation_time_ms": operation_time_ms,
            }
            ...
            operation_time_ms = round((time.perf_counter() - start_time) * 1000, 2)
            logger.error(
                "Error updating intentional memory",
                error=str(e),
                error_type=type(e).__name__,
                operation_time_ms=operation_time_ms,
                correlation_id=correlation_id,
            ...
            return {
                "success": False,
                "error": f"Failed to update intentional memory: {e}",
            }

    def get_self_prompt(self) -> dict[str, Any]:
        """Get the current intentional memory (self-prompt).

        Returns:
            Dict with success status and current intentional memory message
        """
        correlation_id = get_correlation_id() or create_child_correlation_id(
            "get_self_prompt"
        )
        ...
        try:
            ...
            # Get the current intentional memory
            message_bytes = self.client.get(intent_key)
            message = message_bytes.decode("utf-8") if message_bytes else None

            operation_time_ms = round((time.perf_counter() - start_time) * 1000, 2)

            logger.debug(
                "Intentional memory retrieved",
            ...
            operation_time_ms = round((time.perf_counter() - start_time) * 1000, 2)
            logger.error(
                "Error retrieving intentional memory",
                error=str(e),
                error_type=type(e).__name__,
                operation_time_ms=operation_time_ms,
                correlation_id=correlation_id,
            ...
            return {
                "success": False,
                "error": f"Failed to retrieve intentional memory: {e}",
            }
