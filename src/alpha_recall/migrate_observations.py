...
async def migrate_observations():
    ...
    try:
        ...
        for batch_idx in range(total_batches):
            ...
            logger.info(
                f"Processing batch {batch_idx + 1}/{total_batches} ({len(batch)} observations)"
            )
            ...
            for obs in batch:
                ...
                success = await vector_store.store_observation(
                    ...
                    text=content,
                    entity_id=entity_id,
                    metadata={"entity_name": entity_name, "created_at": created_at},
                )

                if not success:
                    logger.warning(
                        f"Failed to migrate observation {observation_id} for entity {entity_name}"
                    )
