...
class MemgraphDatabase(GraphDatabase):
    ...
    async def create_entity(
        ...
        try:
            ...
            if existing:
                ...
                logger.info(f"Entity '{name}' already exists, returning existing entity")
    ...
    async def add_observation(
        ...
        try:
            ...
            if not results_list:
                logger.warning(f"No results returned when adding observation to entity '{entity_name}'")
            ...
            logger.warning(f"No results returned when adding observation, but observation may still have been created: {str(e)}")
    ...
    async def create_relationship(
        ...
        try:
            ...
            # Log success
            logger.info(f"Created relationship '{relationship_type}' from '{source_entity}' to '{target_entity}'")
            ...
            # This specifically handles the list index out of range error
            logger.warning(f"No results returned when creating relationship, but relationship may still have been created: {str(e)}")
            # Try to continue with a successful response
    ...
    async def get_entity(
        ...
        try:
            ...
            obs_results = self.memgraph.execute_and_fetch(obs_query, {"name": entity_name})
            ...
            rel_results = self.memgraph.execute_and_fetch(rel_query, {"name": entity_name})
            ...
            if depth > 1:
                ...
                related_results = self.memgraph.execute_and_fetch(related_query, {"name": entity_name})
