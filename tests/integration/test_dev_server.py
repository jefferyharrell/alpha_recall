...
async def test_dev_server_health_check(dev_server_url):
    ...
    try:
        async with streamablehttp_client(dev_server_url) as (read_stream, write_stream, get_session_id):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                assert timestamp.endswith("+00:00"), f"Timestamp should be UTC format, got: {timestamp}"
